...
CREATE TABLE IF NOT EXISTS jourvolt_wechat_identities (
    app_id TEXT NOT NULL,
    openid_hash TEXT NOT NULL,
    unionid_hash TEXT,
    user_id TEXT REFERENCES jourvolt_users(id) ON DELETE CASCADE,
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    updated_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    PRIMARY KEY (app_id, openid_hash)
);
CREATE TABLE IF NOT EXISTS jourvolt_wechat_link_challenges (
...
func (mockProvider) Status(_ context.Context, userID string, _ int) (vehicleStatus, error) {
	...
	return vehicleStatus{
		...
		Model: "3", TrimBadging: "Development", ExteriorColor: "Pearl White", WheelType: "Aero",
...
func (a *app) status(w http.ResponseWriter, r *http.Request, userID string, carID int, snapshot bool) {
	...
	status := map[string]any{
		...
		"model":          emptyAsNil(providerStatus.Model),
		"trim_badging":   emptyAsNil(providerStatus.TrimBadging),
		"exterior_color": emptyAsNil(providerStatus.ExteriorColor),
		"wheel_type":     emptyAsNil(providerStatus.WheelType),
		"state_since":    nil,
	...
	a.json(w, http.StatusOK, map[string]any{"data": map[string]any{
		...
		"observed_at": providerStatus.ObservedAt.UTC().Format(time.RFC3339),
		"source":      providerStatus.Source, "field_sources": providerStatus.FieldSources,
	}})
}
...
func main() {
	...
	wechatSettings, err := loadWeChatConfig(os.Getenv)
	if err != nil {
		log.Fatal(err)
	}
	...
	a := &app{
		...
		wechat:      newWeChatAuth(wechatSettings, httpClient),
