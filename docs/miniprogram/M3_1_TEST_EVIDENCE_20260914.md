...
| `npm ci --ignore-scripts --no-audit` | PASS | 最终隔离目录 `E:\temp\matelink-m3-clean-install-20260915-final`，实际安装 1183 packages |
...
| `go test ./... -count=1` | PASS with 16 SKIP | JSON：`E:\temp\matelink-m3-evidence-20260915\final-go-test.json`；失败 0，跳过均为缺少 `JOURVOLT_TEST_DATABASE_URL` 的 PostgreSQL 集成路径；新增未知授权错误不泄露内部详情和回调消费后通道保留用例通过 |
