...
代码验证 SHA：`5097a90`（页面分页、授权回调重试、匿名缓存与依赖更新最终回归提交）；最终分支 SHA 在交付报告中记录。
...
- 历史分页：`validPrefix`、`complete`、`hasMore` 和错误原因分开；行程与充电页共用实际调用的分页控制器，合法中间页继续加载，页码/重复页/空页带 hasMore/总页数矛盾不推进成功水位，保留已有真实记录并允许从正确页重试。
